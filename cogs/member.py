...
from bot.context import CommandContext, CommandError
...
class Members(commands.Cog):
    ...
    async def member_root(self, ctx: commands.Context):
        pass

    async def specific_member_root(self, ctx: CommandContext):
    ...
    async def member_info(self, ctx: CommandContext, member: Member):
        await ctx.reply(embed=await libs.embeds.member_card(ctx.conn, member))

    @member_root.command(
        name="new",
        aliases=["create", "add", "register"]
    )
    async def new_member(self, ctx: CommandContext, name: str = None):
        system = await ctx.ensure_system()
        if not name:
        ...
        try:
            member = await system.create_member(ctx.conn, name)
        except PluralKitError as e:
            raise CommandError(e.message)

        await ctx.reply_ok(
            "Member \"{}\" (`{}`) registered! Type `pk;help member` for a list of commands to edit this member.".format(
                name, member.hid))

    @member_root.command(name="set")
    async def member_set(self, ctx: CommandContext):
        raise CommandError(
            "`pk;member set` has been retired. Please use the new member modifying commands. Type `pk;help member` for a list.")

    async def member_name(self, ctx: CommandContext, member: Member):
        system = await ctx.ensure_system()
    ...
    async def member_description(self, ctx: CommandContext, member: Member):
    ...
    async def member_avatar(self, ctx: CommandContext, member: Member):
    ...
    async def member_color(self, ctx: CommandContext, member: Member):
    ...
    async def member_pronouns(self, ctx: CommandContext, member: Member):
    ...
    async def member_birthdate(self, ctx: CommandContext, member: Member):
    ...
    async def member_proxy(self, ctx: CommandContext, member: Member):
        if not ctx.has_next():
            ...
            if "text" not in example:
                raise CommandError(
                    "Example proxy message must contain the string 'text'. For help, type `pk;help proxy`.")

            if example.count("text") != 1:
                raise CommandError(
                    "Example proxy message must contain the string 'text' exactly once. For help, type `pk;help proxy`.")
    ...
    async def member_delete(self, ctx: CommandContext, member: Member):
