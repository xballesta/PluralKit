...
class CommandContext(commands.Context):
    def __init__(self, system: Optional[System] = None, **attrs):
        super().__init__(**attrs)
        self._system = system
        self.bot: pk.PluralKitBot
    ...
    async def has_role(self, role_id: int):
        ...
        for r in author.roles:
            if r.id == role_id:
                return True
        raise PermError(
            f"You don't have the correct permissions to run this command! If you think this is an error, please contact a server admin.")

    async def get_system(self) -> Optional[System]:
        return self._system

    async def ensure_system(self) -> System:
        system = await bot.db.get_system_by_account(self.bot.db, self.author.id)
